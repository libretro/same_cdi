...
void cdi_state::cdimono1_mem(address_map &map)
{
	...
	map(0xe00000, 0xe7ffff).rw(m_dvc, FUNC(cdidvc_device::regs_r), FUNC(cdidvc_device::regs_w));
	map(0xe80000, 0xefffff).rw(m_dvc, FUNC(cdidvc_device::mpeg_ram_r), FUNC(cdidvc_device::mpeg_ram_w));
}
...
{
	save_item(NAME(m_cdic_intreq));
	save_item(NAME(m_dvc_intreq));
	save_item(NAME(m_in4_owner));
}
...
void cdi_state::update_in4_state()
{
	switch (m_in4_owner)
	{
	case IN4_CDIC:
	...
	m_maincpu->in4_w(m_in4_owner != IN4_IDLE ? ASSERT_LINE : CLEAR_LINE);
}

WRITE_LINE_MEMBER(cdi_state::cdic_intreq_w)
{
	m_cdic_intreq = state == ASSERT_LINE;
	update_in4_state();
}

WRITE_LINE_MEMBER(cdi_state::dvc_intreq_w)
{
	m_dvc_intreq = state == ASSERT_LINE;
	update_in4_state();
}

uint8_t cdi_state::intack4_r()
{
	if (m_in4_owner == IN4_CDIC && m_cdic.found())
		return m_cdic->intack_r();
	if (m_in4_owner == IN4_DVC && m_dvc.found())
...
void cdi_state::cdimono1_base(machine_config &config)
{
	...
	m_maincpu->iack4_callback().set(FUNC(cdi_state::intack4_r));
...
ROM_START( cdimono1 )
	...
	ROM_LOAD_OPTIONAL( "vmpega.rom", 0x0000, 0x40000, CRC(db264e8b) SHA1(be407fbc102f1731a0862554855e963e5a47c17b) )
ROM_END
